...
### inject-order
Insert a GM order for a ship or a prefect. Exactly one of `--ship` or `--prefect` must be provided. The command is validated against the chosen subject — attempting to inject a prefect-scoped command (e.g. `CHANGEFACTION`) against a ship returns an error, and vice versa.

| Parameter | Required | Default | Description |
|-----------|:--------:|---------|-------------|
| `--game` | No | `OMICRON101` | Game ID |
| `--ship` | One of | — | Ship ID (mutually exclusive with `--prefect`) |
| `--prefect` | One of | — | Prefect ID (mutually exclusive with `--ship`) |
| `--command` | **Yes** | — | Command string (e.g. `"SCANSYSTEM"` or `"CHANGEFACTION 12 reason text"`) |
