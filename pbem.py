...
def cmd_submit_orders(args):
    ...
    prefect_id = orders.get('prefect', '')
    if not ship_id and not prefect_id:
        print("Error: orders file must specify a ship or prefect ID.")
        return
    if ship_id and prefect_id:
        print("Error: orders file cannot specify both a ship and a prefect.")
        return

    subject_type = 'prefect' if prefect_id else 'ship'
    subject_id = int(prefect_id) if prefect_id else int(ship_id)
    subject_label = str(subject_id)  # used for file naming
    ...
    if subject_type == 'ship':
        valid, account_number, error_msg = folders.validate_ship_ownership(
            email, ship_id, account)
    else:
        # Prefect ownership: check the player email matches the prefect's player
        conn_v = get_connection(db_path)
        pr_row = conn_v.execute(
            "SELECT pr.prefect_id, pl.email, pl.account_number "
            "FROM prefects pr JOIN players pl ON pr.player_id = pl.player_id "
            "WHERE pr.prefect_id = ? AND pr.game_id = ?",
            (subject_id, args.game)
        ).fetchone()
        conn_v.close()
        if not pr_row:
            valid, account_number, error_msg = False, None, f"Prefect {subject_id} not found."
        elif pr_row['email'].lower() != email.lower():
            valid, account_number, error_msg = False, None, (
                f"Email {email} does not own prefect {subject_id}.")
        elif str(pr_row['account_number']) != str(account):
            valid, account_number, error_msg = False, None, (
                f"Account {account} does not match prefect {subject_id}.")
        else:
            valid, account_number, error_msg = True, pr_row['account_number'], None

    if not valid:
        # Store as rejected
        rejected_file, reason_file = folders.store_rejected(
            turn_str, email, subject_label, raw_content,
    ...
    orders_file = folders.store_incoming_orders(turn_str, email, subject_label, raw_content)

    # Write receipt
    receipt_file = folders.store_receipt(turn_str, email, subject_label, {
        'status': 'accepted',
    ...
    if not args.dry_run:
        ...
        # Clear any existing orders for this subject/turn (resubmission replaces)
        conn.execute("""
            DELETE FROM turn_orders
            WHERE game_id = ? AND turn_year = ? AND turn_week = ?
            AND subject_type = ? AND subject_id = ? AND status = 'pending'
        ...
        for o in orders['orders']:
            ...
            """, (
                args.game, game['current_year'], game['current_week'],
                player['player_id'], subject_type, subject_id,
...
def cmd_run_turn(args):
    ...
    # Phase 1.05: Gather prefect-scoped orders (e.g. CHANGEFACTION)
    prefect_orders_map = {}
    prefect_meta = {}  # display_name per prefect
    prefect_rows = conn.execute("""
        SELECT DISTINCT prefect_id FROM prefects WHERE game_id = ?
    """, (args.game,)).fetchall()
    for pr_row in prefect_rows:
        pid = pr_row['prefect_id']
        stored = conn.execute("""
            SELECT * FROM turn_orders
            WHERE game_id = ? AND turn_year = ? AND turn_week = ?
            AND subject_type = 'prefect' AND subject_id = ? AND status = 'pending'
            ORDER BY order_sequence
        """, (args.game, game['current_year'], game['current_week'], pid)).fetchall()
        if not stored:
            continue
        orders_list = []
        for so in stored:
        ...
        prefect_orders_map[pid] = orders_list
        prow = conn.execute(
            "SELECT name FROM prefects WHERE prefect_id = ?", (pid,)
        ).fetchone()
        prefect_meta[pid] = {
            'display_name': prow['name'] if prow else f'Prefect {pid}',
        }
        print(f"  [Prefect] {prefect_meta[pid]['display_name']} ({pid}): {len(orders_list)} orders queued")

    # Phase 1.1: Check for MODERATOR / CHANGEFACTION orders — auto-hold if pending
    mod_orders_found = []
    changefaction_orders_found = []
    # Scan ship orders (MODERATOR only; CHANGEFACTION is now prefect-scoped
    # but we still scan for it defensively in case stale orders exist)
    for sid, orders in ship_orders_map.items():
        ...
        for order in orders:
            if order['command'] == 'MODERATOR':
                ...
                # Legacy / misfiled: treat as prefect-scoped
    ...
    # Scan prefect orders
    for pid, orders in prefect_orders_map.items():
        prow = conn.execute("SELECT name FROM prefects WHERE prefect_id = ?", (pid,)).fetchone()
        for order in orders:
            if order['command'] == 'CHANGEFACTION':
                changefaction_orders_found.append({
                    'ship_id': None,
                    'ship_name': None,
                    'prefect_id': pid,
            ...
            elif order['command'] == 'MODERATOR':
                mod_orders_found.append({
                    'ship_id': None,  # prefect-scoped
                    'ship_name': prow['name'] if prow else f'Prefect {pid}',
                    'prefect_id': pid,
                    'text': order['params']['text'],
                })
    ...
    if mod_orders_found:
        ...
        # ship_id = 0 is a sentinel for prefect-scoped MODERATOR orders
        # (the schema requires ship_id NOT NULL).
        for mo in mod_orders_found:
            ship_id_val = mo['ship_id'] if mo['ship_id'] is not None else 0
            existing = conn.execute("""
                SELECT action_id FROM moderator_actions
                WHERE game_id = ? AND ship_id = ? AND prefect_id = ? AND request_text = ?
                AND requested_turn_year = ? AND requested_turn_week = ?
            """, (args.game, ship_id_val, mo['prefect_id'], mo['text'],
    ...
    # Phase 1.9: Resolve prefect-scoped orders (CHANGEFACTION, prefect MODERATOR)
    # before the ship interleaved pass. These don't consume OCs and have no
    # spatial state — simple dispatch.
    prefect_results = {}
    if prefect_orders_map:
        print(f"\n  Resolving prefect-scoped orders for {len(prefect_orders_map)} prefect(s)...")
        prefect_results = resolver.resolve_prefect_orders(prefect_orders_map)
        # Mark all prefect orders as resolved with their result messages
        for pid, results_list in prefect_results.items():
            for i, res in enumerate(results_list):
                # Find the corresponding stored order by sequence match
                orders_for_pid = prefect_orders_map.get(pid, [])
                if i >= len(orders_for_pid):
                    continue
                seq = orders_for_pid[i].get('sequence', i + 1)
                conn.execute("""
                    UPDATE turn_orders
                    SET status = 'resolved', result_message = ?
                    WHERE game_id = ? AND turn_year = ? AND turn_week = ?
                    AND subject_type = 'prefect' AND subject_id = ?
                    AND order_sequence = ?
                """, (res.get('message', ''), args.game,
                      game['current_year'], game['current_week'],
                      pid, seq))
                display = prefect_meta.get(pid, {}).get('display_name', f'Prefect {pid}')
                status = 'OK' if res.get('success') else '--'
...
def cmd_inject_order(args):
    """Inject a GM order for any ship or prefect."""
    db_path = Path(args.db) if args.db else None
    conn = get_connection(db_path)
    ...
    # Determine subject
    if args.ship and args.prefect:
        print("Error: specify either --ship OR --prefect, not both.")
        conn.close()
        return
    if not args.ship and not args.prefect:
        print("Error: specify either --ship or --prefect.")
        conn.close()
        return

    if args.ship:
        subject_type = 'ship'
        subject_id = args.ship
        row = conn.execute(
            "SELECT s.name, p.player_id FROM ships s "
            "JOIN prefects p ON s.owner_prefect_id = p.prefect_id "
        ...
        subject_id = args.prefect
        row = conn.execute(
            "SELECT pr.name, pr.player_id FROM prefects pr "
            "WHERE pr.prefect_id = ? AND pr.game_id = ?",
            (args.prefect, args.game)
        ).fetchone()
        if not row:
            print(f"Error: Prefect {args.prefect} not found.")
            conn.close()
    ...
    from engine.orders.parser import parse_single_order, command_allowed_for_subject, get_command_subject
    parts = args.command_str.strip().split(None, 1)
    new_cmd = parts[0].upper()
    new_params_raw = parts[1] if len(parts) > 1 else ''
    cmd, params, error = parse_single_order(new_cmd, new_params_raw)
    ...
    # Validate the command is allowed for this subject type
    if not command_allowed_for_subject(cmd, subject_type):
        required = get_command_subject(cmd)
        print(f"Error: {cmd} is a {required}-scoped command and cannot be injected against a {subject_type}.")
        print(f"Use --{required} instead of --{subject_type}.")
        conn.close()
        return

    # Find next sequence number for this subject
    ...
    conn.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')
    """, (args.game, game['current_year'], game['current_week'],
          player_id, subject_type, subject_id, seq_num, cmd, params_json))
    conn.commit()

    print(f"Injected order for {subject_type} {subject_name} ({subject_id}):")
...
Gmail integration (two-stage workflow):
  ...
  send-turns --credentials creds.json        Email turn reports to players
    ...
    sp = subparsers.add_parser('inject-order',
                                help='Inject a GM order for any ship or prefect')
    sp.add_argument('--game', default='OMICRON101', help='Game ID')
    sp.add_argument('--db', help='Path to game_state.db')
    sp.add_argument('--ship', type=int, help='Ship ID (mutually exclusive with --prefect)')
    sp.add_argument('--prefect', type=int, help='Prefect ID (mutually exclusive with --ship)')
